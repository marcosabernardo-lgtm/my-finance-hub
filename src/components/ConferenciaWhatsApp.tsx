...
export default function ConferenciaWhatsApp() {
  ...
  const confirmar = async (r: Rascunho) => {
    ...
    // Marca rascunho como confirmado
    ...
    <div style={{ background: cor.bg, minHeight: '100vh', padding: 24 }}>
      ...
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: cor.sub }}>
            Revise e confirme os lançamentos enviados pelo bot
          </p>
        </div>
      ...
      {rascunhos.length === 0 ? (
        <div style={{ background: cor.card, border: `1px solid ${cor.borda}`, borderRadius: 16, padding: 48, textAlign: 'center' }}>
          ...
          <p style={{ color: cor.sub, fontSize: 13, marginTop: 8 }}>Envie _!gasto [valor] [descrição]_ no WhatsApp para criar um.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {rascunhos.map(r => {
            ...
            return (
              <div key={r.id} style={{ background: cor.card, border: `1px solid ${cor.borda}`, borderRadius: 16, padding: 24, borderLeft: '3px solid #667eea' }}>
                {/* Cabeçalho do card */}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  <div style={{ gridColumn: '1 / -1' }}>
                    <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Descrição</label>
                    <input
                      value={getEdicao(r.id, 'descricao_final', r.estabelecimento) as string}
                      onChange={e => setEdicao(r.id, 'descricao_final', e.target.value)}
                      style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 12px', background: cor.input, border: `1px solid ${cor.borda}`, borderRadius: 8, color: cor.texto, fontSize: 14, fontWeight: 600, outline: 'none', boxSizing: 'border-box' }}
                    />
                  </div>

                  {/* Valor */}
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Valor</label>
                    <input
                      type="number"
                      step="0.01"
                      value={valorAtual}
                      onChange={e => setEdicao(r.id, 'valor_total', e.target.value)}
                      style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 12px', background: cor.input, border: `1px solid ${cor.borda}`, borderRadius: 8, color: cor.texto, fontSize: 14, fontWeight: 700, outline: 'none', boxSizing: 'border-box' }}
                    />
                  </div>

                  {/* Data */}
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Data</label>
                    <p style={{ margin: '4px 0 0', fontSize: 14, color: cor.texto, padding: '8px 12px', background: cor.input, border: `1px solid ${cor.borda}`, borderRadius: 8 }}>{fmtData(r.data_compra)}</p>
                  </div>

                  {/* Categoria */}
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Categoria</label>
                    <select
                      value={categoriaAtual as number || ''}
                      onChange={e => setEdicao(r.id, 'categoria_id', Number(e.target.value))}
                    ...
                    {r.categoria_sugerida && !categoriaAtual && (
                      <p style={{ margin: '4px 0 0', fontSize: 11, color: '#667eea' }}>💡 Sugerida: {r.categoria_sugerida}</p>
                    )}
                  </div>

                  {/* Método de pagamento */}
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Pagamento</label>
                    <select
                      value={metodoAtual}
                      onChange={e => setEdicao(r.id, 'metodo_pagamento', e.target.value)}
                      style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 12px', background: cor.input, border: `1px solid ${cor.borda}`, borderRadius: 8, color: cor.texto, fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                    >
                  ...
                  {!isCredito && (
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Conta</label>
                      <select
                        value={contaAtual as string || ''}
                        onChange={e => setEdicao(r.id, 'conta_origem_destino', e.target.value)}
                        style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 12px', background: cor.input, border: `1px solid ${cor.borda}`, borderRadius: 8, color: cor.texto, fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                      >
                        <option value="">Selecione...</option>
                  ...
                  {isCredito && (
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Cartão</label>
                      <select
                        value={cartaoAtual as number || ''}
                        onChange={e => setEdicao(r.id, 'cartao_id', Number(e.target.value))}
                        style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 12px', background: cor.input, border: `1px solid ${cor.borda}`, borderRadius: 8, color: cor.texto, fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                      >
                        <option value="">Selecione...</option>
                  ...
                  {isCredito && (
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, color: cor.sub, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Parcelas</label>
                      <select
                        value={parcelasAtual}
                        onChange={e => setEdicao(r.id, 'parcelas', Number(e.target.value))}
                        style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 12px', background: cor.input, border: `1px solid ${cor.borda}`, borderRadius: 8, color: cor.texto, fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                      >
                        {[1, 2, 3, 4, 5].map(n => (
                          <option key={n} value={n}>
                            {n === 1 ? 'À vista' : `${n}x ${fmt(valorAtual / n)}`}
                          </option>
                        ))}
                      </select>
                    </div>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 20, justifyContent: 'flex-end' }}>
                  <button
                    ...
                    style={{ padding: '9px 20px', background: 'transparent', border: '1px solid #ef4444', borderRadius: 8, color: '#ef4444', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}
                  >
                    ✕ Rejeitar
                  </button>
                  <button
                    onClick={() => confirmar(r)}
                    disabled={processando === r.id}
                    style={{ padding: '9px 20px', background: '#22c55e', border: 'none', borderRadius: 8, color: 'white', fontWeight: 600, fontSize: 13, cursor: processando === r.id ? 'not-allowed' : 'pointer', opacity: processando === r.id ? 0.7 : 1 }}
                  >
