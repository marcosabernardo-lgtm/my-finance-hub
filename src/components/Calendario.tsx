...
function semanaDomes(data: Date): number {
  const p = new Date(data.getFullYear(), data.getMonth(), 1)
  return Math.ceil((data.getDate() + p.getDay()) / 7)
}

function parseNumeroParcela(np: string | null) {
  if (!np) return null
  const m = np.match(/Parcela (\d+)\/(\d+)/i)
  return m ? { atual: +m[1], total: +m[2] } : null
}

function isCredito(m: Movimentacao) {
  return m.cartao_id !== null || (m.metodo_pagamento || '').toLowerCase().includes('créd')
}

// ── Regras modo Movimentações ─────────────────────────────────────────────────
// Débito/PIX/Dinheiro: só Pago
// Crédito: data_movimentacao !== data_pagamento + só Parcela 1
// Receita: só Pago
// Transferência entre Contas: nunca
...
// Valor a exibir: crédito parcelado mostra valor total
...
function labelCredito(m: Movimentacao): string {
  const parc = parseNumeroParcela(m.numero_parcela)
  if (!parc || parc.total === 1) return 'À vista'
...
function CardMov({ m }: { m: Movimentacao }) {
  ...
  const cor    = m.tipo === 'Receita' ? CORES.receita : cred ? CORES.credito : CORES.debito
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:CORES.card, borderRadius:8, border:`1px solid ${CORES.borda}`, borderLeft:`3px solid ${cor}` }}>
      ...
      <div style={{ fontSize:14, fontWeight:700, whiteSpace:'nowrap', color:cor }}>
...
// ── Card pendente (modo Pendentes) ────────────────────────────────────────────
// Crédito: só nome do cartão + valor
// Débito/PIX: descrição + método + valor
function CardPendente({ m }: { m: Movimentacao }) {
  const cred = isCredito(m)
  const cor  = cred ? CORES.credito : CORES.debito
    ...
    return (
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 12px', background:CORES.card, borderRadius:8, border:`1px solid #fed7aa`, borderLeft:`3px solid ${CORES.credito}` }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <div>
            <div style={{ fontSize:13, fontWeight:600, color:CORES.texto }}>{m.cartoes?.nome || 'Cartão de Crédito'}</div>
            <div style={{ fontSize:10, color:CORES.sub }}>{m.descricao}{m.numero_parcela && m.numero_parcela !== 'Parcela 1/1' ? ` · ${m.numero_parcela}` : ''}</div>
          </div>
        </div>
        <div style={{ fontSize:14, fontWeight:700, color:CORES.credito, whiteSpace:'nowrap' }}>-{fmt(Number(m.valor))}</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', background:CORES.card, borderRadius:8, border:`1px solid #fecaca`, borderLeft:`3px solid ${CORES.debito}` }}>
      ...
      <div style={{ fontSize:14, fontWeight:700, color:cor, whiteSpace:'nowrap' }}>-{fmt(Number(m.valor))}</div>
    </div>
  )
}

// ── Seção expansível por tipo ─────────────────────────────────────────────────
function SecaoTipo({ icone, titulo, cor, movs, modo }: {
  icone: string; titulo: string; cor: string; movs: Movimentacao[]; modo: Modo
}) {
  const [aberto, setAberto] = useState(true)
  if (movs.length === 0) return null
  return (
    <div style={{ border:`1px solid ${cor}22`, borderLeft:`3px solid ${cor}`, borderRadius:8, overflow:'hidden' }}>
      <div onClick={() => setAberto(a => !a)} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'7px 10px', background:`${cor}10`, cursor:'pointer', userSelect:'none' as const }}>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontSize:14 }}>{icone}</span>
          <span style={{ fontSize:11, fontWeight:700, color:cor, textTransform:'uppercase' as const, letterSpacing:'0.5px' }}>{titulo}</span>
          <span style={{ fontSize:10, fontWeight:700, background:cor, color:'#fff', borderRadius:99, padding:'1px 7px' }}>{movs.length}</span>
        </div>
        <span style={{ fontSize:10, color:cor }}>{aberto ? '▲' : '▼'}</span>
      </div>
      {aberto && (
        <div style={{ display:'flex', flexDirection:'column' as const, gap:5, padding:'8px' }}>
          {movs.map(m => modo === 'pendentes' ? <CardPendente key={m.id} m={m} /> : <CardMov key={m.id} m={m} />)}
        </div>
      )}
    </div>
  )
...
function CelulaDia({ dia, movs, semana, isHoje, isMesAtual, modo }: {
  ...
  const totalCredito  = movsExibir.filter(m => m.tipo === 'Despesa' && isCredito(m)).reduce((s,m) => s + (modo === 'pendentes' ? Number(m.valor) : valorExibir(m)), 0)
      ...
      >
        ...
        {temMovs && (
          <div style={{ display:'flex', flexDirection:'column' as const, gap:2 }}>
            ...
            {totalCredito  > 0 && <div style={{ fontSize:10, fontWeight:700, color: isHoje ? '#fed7aa' : CORES.credito }}>-{fmt(totalCredito)} <span style={{ fontSize:9, fontWeight:400, opacity:0.8 }}>cred</span></div>}
...
export default function Calendario() {
  ...
  const fetchDados = useCallback(async () => {
    ...
    // Query 1: movimentações do mês (por data_movimentacao)
    const { data: dataMov } = await supabase
      .from('movimentacoes').select(campos)
      .eq('household_id', householdId)
      .gte('data_movimentacao', `${ano}-${mesStr}-01`)
      .lte('data_movimentacao', `${ano}-${mesStr}-${ultimoDia}`)
    setMovs(norm(dataMov || []))

    // Query 2: pendentes do mês (por data_pagamento, só Despesa Pendente, sem Previsto)
    const { data: dataPend } = await supabase
      .from('movimentacoes').select(campos)
      .eq('household_id', householdId)
      .eq('situacao', 'Pendente')
      .eq('tipo', 'Despesa')
      .neq('metodo_pagamento', 'Transferência entre Contas')
      .gte('data_pagamento', `${ano}-${mesStr}-01`)
      .lte('data_pagamento', `${ano}-${mesStr}-${ultimoDia}`)
    setMovsPend(norm(dataPend || []))
  ...
  const movsPendPorDia = useMemo(() => {
    const map: Record<number, Movimentacao[]> = {}
    for (const m of movsPend) {
      if (!m.data_pagamento) continue
      const dia = parseInt(m.data_pagamento.substring(8, 10))
      if (!map[dia]) map[dia] = []
      map[dia].push(m)
    }
    return map
  ...
  const totalPend     = movsPend.reduce((s,m) => s + Number(m.valor), 0)
  ...
  const rest = 7 - (celulas.length % 7)
  if (rest < 7) for (let d = 1; d <= rest; d++) celulas.push({ dia: d, mes: 'prox' })
    ...
    <div style={{ background:CORES.fundo, minHeight:'100vh', fontFamily:"'Segoe UI',system-ui,sans-serif", padding:'28px 32px' }}>
      ...
      <div style={{ background:CORES.card, borderRadius:16, border:`1px solid ${CORES.borda}`, overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', borderBottom:`2px solid ${CORES.borda}` }}>
          {DIAS_SEMANA.map((d,i) => (
            <div key={d} style={{ padding:'12px 0', textAlign:'center' as const, fontSize:12, fontWeight:700, color:i===0||i===6?CORES.debito:CORES.sub, background:CORES.sepia, textTransform:'uppercase' as const, letterSpacing:'0.5px' }}>
              {d}
            </div>
          ))}
        </div>
      ...
      <div style={{ display:'flex', gap:20, marginTop:16, justifyContent:'center' as const, flexWrap:'wrap' as const }}>
        {modo === 'movimentacoes' ? (
          <>
            {[{cor:CORES.receita,label:'Receita (Pago)'},{cor:CORES.debito,label:'Débito/PIX (Pago)'},{cor:CORES.credito,label:'Crédito (valor total)'},{cor:CORES.hoje,label:'Hoje'}].map(({cor,label})=>(
          ...
          <>
            {[{cor:CORES.debito,label:'Débito/PIX Pendente'},{cor:CORES.credito,label:'Crédito Pendente'},{cor:CORES.hoje,label:'Hoje'}].map(({cor,label})=>(
              <div key={label} style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:CORES.sub }}>
                <div style={{ width:10, height:10, borderRadius:2, background:cor }}/>
                {label}
              </div>
