  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text-1)', margin: 0 }}>Fatura do Cartão</h1>
      ...
      {/* Filtros */}
      <div style={{ background: 'var(--bg-row)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px 20px', marginBottom: '20px', display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {(['', 'Pendente', 'Previsto'] as const).map(s => (
              <button key={s} onClick={() => setFiltroSituacao(s)} style={{
                ...
                border: filtroSituacao === s ? 'none' : '1px solid var(--border-input)',
                background: filtroSituacao === s
                  ? s === '' ? '#2563eb' : s === 'Pendente' ? '#92400e' : '#6b21a8'
                  : 'var(--bg-card)',
                color: filtroSituacao === s ? '#fff' : 'var(--text-4)',
              }}>
                {s === '' ? 'Todos' : s}
              </button>
            ))}
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
        ...
        {/* Cabeçalho da tabela */}
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg-row)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <span style={{ fontSize: '12px', color: 'var(--text-3)' }}>Filtrado por data de pagamento</span>
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: 'var(--bg-row)', borderBottom: '1px solid var(--border)' }}>
                  ...
                  {['Dt. Movimentação','Dt. Pagamento','Categoria','Descrição','Valor','Forma de Pgto','Nº Parcela','Situação'].map(h => (
                    <th key={h} style={{ padding: '10px 12px', textAlign: h === 'Valor' ? 'right' : 'left', fontWeight: 600, color: 'var(--text-4)', whiteSpace: 'nowrap', fontSize: '12px' }}>
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {lancamentosFiltrados.map((l, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: conferido ? 'var(--text-2)' : '#991b1b', whiteSpace: 'nowrap' }}>
                        {editandoValor === l.id ? (
                          <input
                      ...
                      <td style={{ ...tdStyle, color: 'var(--text-2)' }}>{l.forma_pagamento || '—'}</td>
                      <td style={{ ...tdStyle, color: 'var(--text-2)' }}>{l.numero_parcela || '—'}</td>
                      <td style={tdStyle}>
                        <select
                          value={l.situacao}
              ...
              <tfoot>
                <tr style={{ background: 'var(--bg-row)', borderTop: '2px solid var(--border)' }}>
                  <td />
                  <td colSpan={4} style={{ padding: '10px 12px', fontWeight: 700, color: 'var(--text-4)', fontSize: '13px' }}>
